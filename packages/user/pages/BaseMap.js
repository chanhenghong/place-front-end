...
const BaseMap = ({ MapStyles, defaultCenter, zoom }) => {
  ...
  React.useEffect(() => {
    ...
    function onDragEnd() {
      ...
      let maps={latlong:[lngLat.lat,lngLat.lng]}
      // let data = JSON.parse(localStorage.getItem("dataStorage"));
      // data={...data,...maps}
      // localStorage.setItem('dataStorage',JSON.stringify(data))
      setContentData({...contentData,...maps})
      console.log("Data",{...contentData,...maps})
    }
  ...
  return (
    <>
      <div className={classes.root}>
        <PersistentDrawerLeft style={{paddingTop:"200px"}}></PersistentDrawerLeft>
      </div>
