...
const ArticleContentPageVideo = () => {
  const classes = style();
  const [articlePages, SetArticlePages] = React.useState([]);
  const [buttonColor, setButtonColor] = React.useState("right");
...
`;
  ...
  const handleButtonColor = (event, newColor) => {
    if (newColor !== null) {
...
export default ArticleContentPageVideo;
